...
const showProxyOptions = ref(false)
...
const recoveryMethods = [
  { id: 'email', name: 'Email Verification', description: 'Get code at recovery email', estimatedTime: '1-3 min' },
  { id: 'phone', name: 'Phone/SMS Verification', description: 'Get code via text message', estimatedTime: '1-5 min' },
  { id: 'trusted', name: 'Trusted Contacts', description: 'Contact designated friends', estimatedTime: '5-15 min' },
  { id: 'id', name: 'Photo ID Verification', description: 'Submit government-issued ID', estimatedTime: '15-48 hrs' }
]
...
function goToStep(step) {
  ...
  if (step === 5) showSuccessModal.value = true
}
...
function startAnalysis() {
  ...
  analysisInterval = setInterval(() => {
    if (currentIndex < analysisStatus.length - 1) {
      currentIndex++
      currentAnalysisIndex.value = currentIndex
    } else {
      clearInterval(analysisInterval)
      analysisComplete.value = true
      currentAnalysisIndex.value = analysisStatus.length
    }
  }, 1200)
}
...
function startProcessing() {
  ...
  const interval = setInterval(() => {
    ...
    progress.value += 2
    if (progress.value >= 15 && currentStatusIndex.value < 1) currentStatusIndex.value = 1
    else if (progress.value >= 35 && currentStatusIndex.value < 2) currentStatusIndex.value = 2
    else if (progress.value >= 55 && currentStatusIndex.value < 3) currentStatusIndex.value = 3
    else if (progress.value >= 75 && currentStatusIndex.value < 4) currentStatusIndex.value = 4
    else if (progress.value >= 90 && currentStatusIndex.value < 5) currentStatusIndex.value = 5
  }, 300)
}
...
// lifecycle
onMounted(() => {
  console.log('Mounted, ready to start process...')
})
